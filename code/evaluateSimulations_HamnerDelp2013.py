...
@author:
    ...
    This script runs through the process of evaluating the RRA and Moco Tracking
...
for subject in subList:
    ...
    # %% Evaluate residuals using inverse simulations
    
    # #Check whether to run inverse simulations
    # if runInverseSimulations:
        
    #     #Set up folder for inverse simulations
    #     try:
    #         os.mkdir(f'..\\data\\HamnerDelp2013\\{subject}\\inverse\\')
    #         os.mkdir(f'..\\data\\HamnerDelp2013\\{subject}\\inverse\\{runLabel}')
    #     except:
    #         print(f'Inverse folder already created for {subject} and {runLabel}...')
            
    #     #### TODO: set-up file sink logger
        
    #     #Change directory
    #     os.chdir(f'..\\data\\HamnerDelp2013\\{subject}\\inverse\\{runLabel}')
    ...
    #     # parameters.
    #     modelProcessor = osim.ModelProcessor(f'{subject}_{runLabel}_cycle1_rraAdjusted.osim')
    ...
    #     # #Edit the weird data source stuff going on with the external loads
    #     # #### TODO: saved and manually deleted for now
    #     # inverseModel = osim.Model('test.osim')
        
    #     #Add in torque actuators that replicate the RRA actuators
    #     #### TODO: need to add these dictionaries back into set-up if using
    ...
    #     # inverseModel.finalizeFromProperties()
    #     # inverseModel.printToXML('test.osim')
        
    #     #Set model in inverse tool
    #     inverse.setModel(osim.ModelProcessor(inverseModel))
    #     # inverse.setModel(osim.ModelProcessor('test.osim'))
    ...
    #     # base table.
    #     # inverse.setKinematics(osim.TableProcessor(f'{subject}_{runLabel}_cycle1_states.sto'))
    #     inverse.setKinematics(osim.TableProcessor(f'{subject}_{runLabel}_cycle1_mocoKinematics.sto'))
    #     # inverse.setKinematics(osim.TableProcessor(f'{runName}_coordinates.sto'))
        
    #     # Initial time, final time, and mesh interval.
    #     inverse.set_initial_time(osim.TimeSeriesTable(f'{subject}_{runLabel}_cycle1_states.sto').getIndependentColumn()[0])
    #     inverse.set_final_time(osim.TimeSeriesTable(f'{subject}_{runLabel}_cycle1_states.sto').getIndependentColumn()[-1])
    ...
    #     # solution.getMocoSolution().write('solutionMoco.sto')
    
# %% ----- end of evaluateSimulations_HamnerDelp2013.py ----- %% #